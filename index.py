...
def api_check():
    ...
    try:
        ...
        # 4. Status Keamanan Akurat (Email & 2FA)
        email_status = "Unverified"
        email_res = session.get('https://accountsettings.roblox.com/v1/email', headers=headers)
        if email_res.status_code == 200:
            ed = email_res.json()
            if ed.get('verified') or ed.get('emailAddress'):
                email_addr = ed.get('emailAddress', '')
                email_status = f"Verified ({email_addr[:3]}***)" if email_addr else "Verified"

        has_a2f = False
        two_step_res = session.get(f'https://twostepverification.roblox.com/v1/users/{user_id}/settings', headers=headers)
        if two_step_res.status_code == 200:
            ts_data = two_step_res.json()
            # Mengecek berbagai jenis status 2FA yang aktif
        ...
        # 5. Cek Item Katalog (Headless, Korblox, VFX, Limited)
        ...
        # 6. Filter Animasi Aktual (Hanya menampilkan aset kategori animasi/bundle)
        animations = []
        ...
        if avatar_wearing.status_code == 200:
            ...
            for asset in assets_list:
                ...
                said = asset.get('id')
                # Filter kata kunci animasi yang umum dipakai
                if any(k in aname.lower() for k in ['climb', 'fall', 'idle', 'jump', 'run', 'swim', 'walk', 'animation', 'pack']):
                    animations.append({"name": aname, "icon": get_asset_icon(said)})

        if not animations:
            animations.append({"name": "No Animation Package Equipped", "icon": avatar_url})
        ...
        # 7. Riwayat Game & Spent Game dari Endpoint Transaksi Pembelian & Presence Riil
        game_spent = []
        game_history = []

        # Ambil Presence Game Terakhir
        ...
        if pres_res.status_code == 200:
            ...
            if pres_list:
                p_item = pres_list[0]
                place_id = p_item.get('placeId')
                root_place_id = p_item.get('rootPlaceId')
                last_loc = p_item.get('lastLocation')
                
                if last_loc and last_loc != "Website":
                    game_history.append({"name": last_loc, "icon": avatar_url})
                elif place_id:
                    # Ambil nama game dari Universe/Place API
                    game_info = session.get(f'https://games.roblox.com/v1/games/multiget-place-details?placeIds={place_id}', headers=headers)
                    if game_info.status_code == 200:
                        g_data = game_info.json()
                        if g_data:
                            g_name = g_data[0].get('name', 'Roblox Experience')
                            game_history.append({"name": g_name, "icon": avatar_url})

        if not game_history:
            game_history.append({"name": "Website / Offline", "icon": avatar_url})

        # Ambil Transaksi In-Game Purchases Riil
